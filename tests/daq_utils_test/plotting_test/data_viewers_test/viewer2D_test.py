...
import pymodaq.daq_utils.plotting.filter
import pymodaq.daq_utils.plotting.plot_utils
...
class TestData0DWithHistory:
    def test_add_datas_list(self, init_qt):
        Nsamplesinhisto = 2
        data_histo = pymodaq.daq_utils.plotting.plot_utils.Data0DWithHistory(Nsamplesinhisto)
    ...
    def test_add_datas(self, init_qt):
        data_histo = pymodaq.daq_utils.plotting.plot_utils.Data0DWithHistory()
...
class TestExtractAxis:
    def test_info_data_is_None(self):
        ...
        assert pymodaq.daq_utils.plotting.plot_utils.AxisInfosExtractor.extract_axis_info(axis) == (1, 0, 'mylabel', 'myunits')

    def test_info(self):
        axis = utils.Axis(label='mylabel', units='myunits', data=np.array([5, 20, 35]))
        assert pymodaq.daq_utils.plotting.plot_utils.AxisInfosExtractor.extract_axis_info(axis) == (15, 5, 'mylabel', 'myunits')

    def test_info_data_is_ndarray(self):
        axis = np.array([5, 20, 35])
        assert pymodaq.daq_utils.plotting.plot_utils.AxisInfosExtractor.extract_axis_info(axis) == (15, 5, '', '')

    def test_info_data_is_ndarray_scalingisneg(self):
        axis = np.array([35, 20, 5])
        assert pymodaq.daq_utils.plotting.plot_utils.AxisInfosExtractor.extract_axis_info(axis) == (-15, 35, '', '')

    def test_info_neg_scaling(self):
        axis = utils.Axis(label='mylabel', units='myunits', data=np.array([35, 20, 5]))
        assert pymodaq.daq_utils.plotting.plot_utils.AxisInfosExtractor.extract_axis_info(axis) == (-15, 35, 'mylabel', 'myunits')


class TestLineoutData:
    def test_with_error(self):
        with pytest.raises(ValueError):
            pymodaq.daq_utils.plotting.filter.LineoutData(hor_axis=np.random.random(10), hor_data=np.random.random(12))

        with pytest.raises(ValueError):
            pymodaq.daq_utils.plotting.filter.LineoutData(ver_axis=np.random.random(10), ver_data=np.random.random(12))

    def test_intdataisnotnone(self):
        pymodaq.daq_utils.plotting.filter.LineoutData(ver_axis=np.random.random(10), ver_data=np.random.random(10), int_data=np.array([10]))

    def test_intdataisnone(self):
        pymodaq.daq_utils.plotting.filter.LineoutData(ver_axis=np.random.random(10), ver_data=np.random.random(10))
